...
def build_export_dataframe(job_id=None, keyword=None, score_min=None, score_max=None,
                           ...
                           min_reviews=None, city=None, language=None, companies=None):
...
def start_scrape():
    ...
    check_gmb = data.get('check_gmb', False)
    ...
    thread = threading.Thread(
        ...
        args=(job_id, keyword, max_pages, max_companies, start_page, include_independents,
              check_websites, check_moneyhouse, check_architectes, check_bienvivre, check_zip, check_gmb)
    )
    thread.daemon = True
    thread.start()
...
def start_pipedrive_import(job_id):
    ...
    job_companies = list(
        companies_collection.find({'job_id': ObjectId(job_id)}).sort('credibility_score', -1)
    )
    if not job_companies:
        return jsonify({'error': 'No companies available to import'}), 400

    # §5.3: skip creates that match prior companies (website/MoneyHouse → phone → fuzzy name)
    # and collapse duplicates within this job before sending rows to Pipedrive.
    existing_companies = list(
        companies_collection.find({'job_id': {'$ne': ObjectId(job_id)}})
    )
    dedupe_result = dedupe_for_pipedrive_create(job_companies, existing_companies)
    create_rows = dedupe_result['create_rows']
    if not create_rows:
        return jsonify({
            'error': 'All companies were skipped by duplicate checks (website/MoneyHouse, phone, or fuzzy name)',
            'local_dedupe': {
                'rows_before': dedupe_result['rows_before'],
                'rows_after': 0,
                'skipped_duplicates': dedupe_result['skipped_duplicates'],
            }
        }), 400
    ...
    jobs_collection.update_one(
        ...
        {'$set': {
            'pipedrive_dedupe': {
                'rows_before': dedupe_result['rows_before'],
                'rows_after': dedupe_result['rows_after'],
                'skipped_count': len(dedupe_result['skipped_duplicates']),
                'skipped_duplicates': dedupe_result['skipped_duplicates'][:100],
    ...
    if isinstance(payload, dict):
        payload['local_dedupe'] = {
            'rows_before': dedupe_result['rows_before'],
            'rows_after': dedupe_result['rows_after'],
            'skipped_count': len(dedupe_result['skipped_duplicates']),
            'skipped_duplicates': dedupe_result['skipped_duplicates'][:50],
        }
