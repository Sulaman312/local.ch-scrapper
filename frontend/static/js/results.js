...
function renderResults(companies) {
    ...
    tbody.innerHTML = companies.map(company => {
        const phones = company.landline_numbers
            ? company.landline_numbers.split(',').map(p => p.trim()).filter(Boolean)
            : (company.phone_numbers ? company.phone_numbers.split(',').map(p => p.trim()).filter(Boolean) : []);
        const mobiles = company.mobile_numbers
            ? company.mobile_numbers.split(',').map(p => p.trim()).filter(Boolean)
            : [];
        ...
        return `
            <tr>
                ...
                <td>${renderMultiValue(mobiles, 'phone', company._id)}</td>
                ...
                <td>${renderGmbCell(company)}</td>
                <td>${formatGmbValue(company.gmb_rating)}</td>
                <td>${formatGmbValue(company.gmb_review_count, 0)}</td>
                <td>${renderSocialMedia(company)}</td>
                <td>${renderBooleanBadge(company.has_local_search)}</td>
                <td>${renderBooleanBadge(company.has_localch_banner_ads)}</td>
...
function formatGmbValue(value, emptyFallback = 'N/A') {
...
function renderGmbCell(company) {
    if (company.has_gmb === 'N/A' || company.gmb_url === 'N/A') {
        return '<span class="badge" style="background: #f3f4f6; color: #6b7280;">N/A</span>';
    }
    if (company.gmb_url) {
        const label = company.gmb_name || 'Google Business Profile';
        return `<a href="${company.gmb_url}" target="_blank" rel="noopener noreferrer" style="color: var(--navbar-secondary); text-decoration: none;" title="${label}">
            <i class="fab fa-google"></i> Open
        </a>`;
    }
    return renderBooleanBadge(!!company.has_gmb);
}
...
async function viewCompany(companyId) {
    try {
        ...
        const phones = normalizeModalList(company.landline_numbers || company.phone_numbers || company.phone_numbers_raw);
        const mobiles = normalizeModalList(company.mobile_numbers);
        const emails = company.email ? [company.email] : [];
            ...
            <div style="display: grid; gap: 1.5rem;">
                ...
                <div style="background: var(--bg-light); padding: 1.5rem; border-radius: 0.75rem; border: 1px solid var(--border-gray);">
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        ...
                        <tr style="border-bottom: 1px solid var(--border-gray);">
                            <td style="padding: 0.75rem 0; font-weight: 600; vertical-align: top;">Mobile</td>
                            <td style="padding: 0.75rem 0;">
                                ${mobiles.length > 0 ? mobiles.map(p => `<div style="margin: 0.25rem 0;"><i class="fas fa-mobile-alt" style="color: var(--navbar-secondary); width: 1.25rem;"></i> ${p}</div>`).join('') : 'N/A'}
                            </td>
                        </tr>
                        <tr style="border-bottom: 1px solid var(--border-gray);">
                ...
                <div style="background: var(--bg-light); padding: 1.5rem; border-radius: 0.75rem; border: 1px solid var(--border-gray);">
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        ...
                        <tr style="border-bottom: 1px solid var(--border-gray);">
                            <td style="padding: 0.75rem 0; font-weight: 600;">Google Business Profile</td>
                            <td style="padding: 0.75rem 0;">${renderGmbCell(company)}</td>
                        </tr>
                        <tr style="border-bottom: 1px solid var(--border-gray);">
                            <td style="padding: 0.75rem 0; font-weight: 600;">GMB Rating</td>
                            <td style="padding: 0.75rem 0;">${formatGmbValue(company.gmb_rating)}</td>
                        </tr>
                        <tr style="border-bottom: 1px solid var(--border-gray);">
                            <td style="padding: 0.75rem 0; font-weight: 600;">GMB Reviews</td>
                            <td style="padding: 0.75rem 0;">${formatGmbValue(company.gmb_review_count)}</td>
                        </tr>
                        <tr style="border-bottom: 1px solid var(--border-gray);">
                        ...
                        <tr style="border-bottom: 1px solid var(--border-gray);">
                            <td style="padding: 0.75rem 0; font-weight: 600;">Web Banner Ads (manual)</td>
                            <td style="padding: 0.75rem 0;">${renderBooleanBadge(company.has_web_banner_ads)}</td>
                        </tr>
                        <tr style="border-bottom: 1px solid var(--border-gray);">
...
async function startPipedriveImport() {
    ...
    try {
        ...
        const skipped = data.local_dedupe?.skipped_count || 0;
        const after = data.local_dedupe?.rows_after;
        let queuedMessage = window.i18n.t('results.pipedriveQueued');
        if (data.local_dedupe) {
            queuedMessage = window.i18n.t('results.pipedriveDedupeQueued', {
                after: after ?? '?',
                before: data.local_dedupe.rows_before ?? '?',
                skipped
            });
        }
        updatePipedriveImportPanel({
